...
workflow copyDatabases {
	...
	# Copy databases.
	foreach -Parallel ($db in $databasesToCopy) {
		Import-AzureRmContext -Path $profilePath
	
		$newName = ($db.DatabaseName -replace $versionedDbMatch, "`$1")  + $newVersion
		Write-Progress -Activity "Copying database $($db.DatabaseName) to $newName"
		$serverName = $db.ServerName
		Write-Progress -Activity "Copying from and to server $serverName"
		...
		if ($existing) {
			Write-Progress -Activity "Database with name $newName alread exists, skipping copy."
		}
		else {
			Write-Progress -Activity "Copy database."
...
copyDatabases -databasesToCopy $databases -versionedDbMatch $versionedDbMatch -newVersion $NewVersion -resourceGroupName $ResourceGroupName -profilePath $profilePath
