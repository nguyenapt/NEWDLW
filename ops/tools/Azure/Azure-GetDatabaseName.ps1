<#
.SYNOPSIS
Gets the most recent database name on all detected servers of versioned databases and puts it in a variable.
...
$versions = Get-AzureRmSqlServer -ResourceGroupName $ResourceGroupName | Get-AzureRmSqlDatabase `
    ...
    | select DatabaseName, @{ Name = "Version"; Expression= {[Convert]::ToDecimal("$($Matches[2])", $invariantCulture)} } `
	| sort Version -Descending

if ($versions) {
    [Environment]::SetEnvironmentVariable($VariableName, $versions[0].DatabaseName, "Process")
    Write-Host "##vso[task.setvariable variable=$VariableName]$($versions[0].DatabaseName)"
    Write-Host "Found database version $($versions[0].DatabaseName) and set it in variable $VariableName."
}
else {
